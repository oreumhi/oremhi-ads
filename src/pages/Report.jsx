...
const STD_HEAD = ['노출수', '클릭수', '클릭률', '구매완료 전환율', '총비용', '구매완료 전환수', '구매완료비용', '구매완료 전환매출액', '구매완료 ROAS'];
const stdCells = (m) => [
  { v: fmtNum(Math.round(m.impressions || 0)) }, { v: fmtNum(Math.round(m.clicks || 0)) }, { v: ctrOf(m).toFixed(2) + '%' },
  { v: cvrOf(m).toFixed(1) + '%' }, { v: won(m.cost), color: '#f59f3b', bold: true }, { v: fmtNum(Math.round(m.conversions || 0)), color: '#12b886', bold: true },
...
const METRICS = [
  ...
  { key: 'conversions', label: '구매완료 전환수', get: (m) => m.conversions, fmt: num },
  { key: 'cvr', label: '구매완료 전환율', get: cvrOf, fmt: (v) => v.toFixed(1) + '%' },
  { key: 'cpa', label: '구매완료비용(CPA)', get: cpaOf, fmt: won, invert: true },
  { key: 'revenue', label: '구매완료 전환매출액', get: (m) => m.revenue, fmt: won },
  { key: 'roas', label: '구매완료 ROAS', get: roasOf, fmt: roasStr },
];
...
export default function Report({ currentUser, allowedBrands }) {
  ...
  const demoAgg = useCallback((dim, sortMode) => {
    ...
    if (sortMode === 'age') {
      const av = (val) => {
        const t = String(val);
        if (t.includes('알 수 없') || t.includes('알수없')) return 100000;
        const mm = t.match(/\d+/); let n = mm ? +mm[0] : 99999;
        if (t.includes('미만')) n -= 0.5;   // '14세 미만'을 '14세~18세'보다 위로
        return n;
      };
      arr.sort((a, b) => av(a.label) - av(b.label));
  ...
  const summary = useMemo(() => {
    ...
    parts.push(`광고비 ${won(cur.cost)}(${gCost >= 0 ? '+' : ''}${gCost.toFixed(0)}%), 구매완료 매출 ${won(cur.revenue)}(${gRev >= 0 ? '+' : ''}${gRev.toFixed(0)}%), 구매완료 전환 ${num(cur.conversions)}건(${gConv >= 0 ? '+' : ''}${gConv.toFixed(0)}%)을 기록했습니다.`);
  ...
  const kakaoText = useMemo(() => {
    ...
    return [
      ...
      `· 광고비 ${won(cur.cost)}  · 구매완료 매출 ${won(cur.revenue)}`,
      `· 구매완료 ROAS ${roasStr(roasOf(cur))}  · 구매완료 전환 ${num(cur.conversions)}건 (구매완료 전환율 ${cvrOf(cur).toFixed(1)}%)`,
  ...
  const buildStyledWb = (X) => {
    ...
    if (channel === 'search' && kwAgg.length) {
      // 화면과 같은 순서 — 자주 비는 항목(효율·숨은보석)을 뒤로
      if (topKw.length) xAddTable(wb, '키워드-매출TOP10', ['키워드', ...XHEAD], topKw.map(k => [k.keyword, ...xr(k.m)]), XMETA, { sub: kwHead });
      if (costKw.length) xAddTable(wb, '키워드-광고비TOP10', ['키워드', ...XHEAD], costKw.map(k => [k.keyword, ...xr(k.m)]), XMETA, { sub: '예산이 어디로 갔나' });
  ...
  const typeHead = ['광고유형', '노출', '클릭', 'CTR', 'CPC', '광고비', '구매전환', '구매CVR', '구매매출', '구매ROAS', '비중'];
      ...
      <div className="report-sheet" style={{ background: '#fff', color: R.ink, borderRadius: 14, boxShadow: '0 6px 30px rgba(0,0,0,0.35)', maxWidth: 900, margin: '0 auto', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '24px 30px' }}>
          ...
          {/* 키워드 요약 — 전 키워드는 실을 수 없어 관점별 TOP10으로 (검색광고만)
              데이터가 없는 블록은 빼고, 남은 블록에만 ①②③… 번호를 다시 매긴다.
              (2026-08-04 대표님 지적: 빠진 블록 때문에 번호가 건너뜀)
              순서도 '자주 비는 항목'을 뒤로 — 효율·숨은보석은 기준선 때문에 자주 비어 있음 */}
          {channel === 'search' && kwAgg.length > 0 && (() => {
            const NUM = ['①', '②', '③', '④', '⑤', '⑥'];
            const blocks = [
              topKw.length && { color: R.pink, title: '매출 기여 TOP 10 — 어디서 돈을 벌었나', rows: topKw, head: [...STD_HEAD],
                cells: (k) => stdCells(k.m) },
              costKw.length && { color: R.warn, title: '광고비 TOP 10 — 예산이 어디로 갔나', rows: costKw, head: [...STD_HEAD, '비중'],
                cells: (k) => [...stdCells(k.m), { v: (kwTotal.cost > 0 ? k.m.cost / kwTotal.cost * 100 : 0).toFixed(1) + '%', color: R.sub }] },
              actionKw.length && { color: R.no, title: '조치 대상 — 전환 0 또는 ROAS 100% 미만', rows: actionKw, head: [...STD_HEAD],
                note: '비용은 나갔지만 본전을 넘기지 못한 키워드입니다. 입찰 조정·제외 또는 랜딩 점검 대상입니다.', cells: (k) => stdCells(k.m) },
              roasKw.length && { color: R.ok, title: `효율 TOP 10 — 클릭 ${KW_MIN_CLICKS}회 이상만`, rows: roasKw, head: [...STD_HEAD],
                note: '클릭이 적어 우연히 효율이 높게 잡힌 키워드는 제외했습니다. 다음 기간에도 재현될 가능성이 높은 키워드입니다.', cells: (k) => stdCells(k.m) },
              gemKw.length && { color: R.pur, title: '숨은 보석 — 적게 쓰고 잘 번 키워드 (증액 후보)', rows: gemKw, head: [...STD_HEAD],
                note: `광고비는 전체의 1% 미만인데 효율은 평균(${roasStr(roasOf(kwTotal))})의 2배 이상입니다. 예산을 더 넣으면 매출이 늘어날 여지가 있습니다.`, cells: (k) => stdCells(k.m) },
            ].filter(Boolean);
            ...
            return (
              <Section title="키워드 요약" sub={kwHead}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                  {blocks.map((b, i) => (
                    <div key={b.title}>
                      <div style={{ fontSize: 12.5, fontWeight: 700, color: b.color, marginBottom: 6 }}>{NUM[i]} {b.title}</div>
                      {b.note && <div style={{ fontSize: 11, color: R.sub, marginBottom: 6 }}>{b.note}</div>}
                      <MetricTable head={['키워드', ...b.head]} rows={b.rows.map(k => ({ label: k.keyword, cells: b.cells(k) }))} />
                    </div>
                  ))}
                </div>
              </Section>
